...
- [ ] **[Critical] Disable audience bypass** — `verify_aud: False` in `api/src/auth.py:29` means any Keycloak-issued token (for any client) is accepted; set the correct `audience` value and remove the override
- [ ] **[High] JWKS cache TTL** — `api/src/auth.py:9` caches JWKS keys forever; add a TTL (e.g. 5 min) and retry on decode failure to handle Keycloak key rotation gracefully
- [ ] **[High] Distinguish auth vs infra errors** — `api/src/auth.py:30` catches all exceptions and returns 401; a Keycloak outage should return 503, not 401
- [ ] **[Medium] Server-side MIME detection for uploads** — `api/src/routes.py:33` checks `file.content_type` from the client header, which is attacker-controlled; use `python-magic` to inspect actual file bytes
- [ ] **[Medium] Paginate `/promises`** — `api/src/routes.py:24-27` returns the entire collection unbounded; add `limit`/`skip` parameters with a max page size to prevent memory exhaustion

## Frontend

- [ ] **[Critical] Replace `admin-cli` + password grant** — `Login.tsx:21-22` uses the Keycloak admin client with the deprecated ROPC flow; create a dedicated app client in Keycloak and switch to Authorization Code + PKCE
- [ ] **[Medium] CSP headers for token in sessionStorage** — the JWT is stored in `sessionStorage`, accessible to any JS on the page; add strict `Content-Security-Policy: script-src 'self'` headers at the gateway/ingress level
...
- [ ] **[Critical] Keycloak `start-dev` in production** — `helm/charts/iam/templates/deployment.yaml:20` runs `start-dev` in all envs; this disables TLS and uses an in-memory H2 DB (state lost on pod restart); switch to `start` with an external DB for prod
- [ ] **[High] Keycloak admin credentials in Helm values** — `helm/charts/iam/templates/deployment.yaml:29-31` injects the admin password from plain values (visible in `helm history`); move to a Kubernetes `Secret` and use `valueFrom.secretKeyRef`
...
- [ ] **Celery concurrency silently follows node CPU count, not `values.concurrency`** — since `CELERY_CONCURRENCY` is inert (see `helm/apps/worker/values.yaml:57`), Celery's prefork pool defaults to `os.cpu_count()` of whichever node the pod lands on, not the pod's CPU *limit*. Staging nodes range from 2 to 8 cores, so the same pod can fork 2 or 8 worker processes depending on scheduling — on the 8-core node both `worker` replicas OOMKilled repeatedly under the old 256Mi limit. Memory bumped to 1Gi (`argocd/clusters/staging/values/worker.enc.yaml`, via `sops`) as a stopgap; the real fix is upstream (`kubernetes-ai-system`'s `main.py` reading `CELERY_CONCURRENCY`/`CELERY_POOL` and passing `--concurrency`), since only then does memory usage become predictable regardless of node.

## General

### Persistent Volumes & Claims (Database + LLM)
