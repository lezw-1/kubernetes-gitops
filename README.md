...
- **LLM (`helm/apps/llm`)** — self-hosted Ollama Helm chart: Deployment (init container pulls the configured model on startup), Service, PV/PVC. Enabled on dev only (`replicas: 1`); disabled on staging/prod (`replicas: 0`), which call the Anthropic API directly instead, same as `worker`.
